...
用法：
   python src/tools/convert_policy.py <model_file_path>

说明：
- 输入 .pt 时自动导出同名 .onnx。
- 输入 .onnx 时自动导出同名 .pt。
- .pt 文件需要是 TorchScript 模型。
- 转换后会自动比较输出误差 max_diff。
- 依赖缺失时安装：
     pip install onnx onnxruntime onnxscript onnx2torch
- ONNX 转回 .pt 可能丢失原始模块结构，建议保留原始 .pt。
"""
...
def convert_pt_to_onnx(pt_path, onnx_path):
    ...
    torch.onnx.export(
        ...
        dynamo=False                # TorchScript models need the legacy exporter
    )
