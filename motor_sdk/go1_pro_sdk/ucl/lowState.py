from .enums import MotorModeLow, GaitType, SpeedLevel
from enum import Enum
from .common import float_to_hex, hex_to_float, tau_to_hex, hex_to_tau, encryptCrc, genCrc, byte_print, hex_to_float_2B
from .complex import cartesian, led, bmsState, imu, motorState, wirelessRemoteState
import struct
...
class lowState:
    def __init__(self): #highState len == 1087 / lowState len == 807
        ...
        self.wirelessRemoteState = None  # parsed wireless remote state
    ...
    def dataToWirelessRemoteState(self, data):
        """
        Parse wireless remote state from data bytes.
        data: bytearray or bytes of length at least 26
        Returns: wirelessRemoteState object
        """
        head = [data[0], data[1]]
        ...
        # Joystick values (2 bytes each)
        lx = hex_to_float_2B(data, 18, 19)
        rx = hex_to_float_2B(data, 20, 21)
        ry = hex_to_float_2B(data, 22, 23)
        ly = hex_to_float_2B(data, 24, 25)  # TODO may be not right. Only use 24 B
        
        return wirelessRemoteState(head, btn_components, lx, rx, ry, ly)
    ...
    def parseData(self, data):
        ...
        # Parse wireless remote state if data is available
        if len(self.wirelessRemote) >= 26:
            self.wirelessRemoteState = self.dataToWirelessRemoteState(self.wirelessRemote)
