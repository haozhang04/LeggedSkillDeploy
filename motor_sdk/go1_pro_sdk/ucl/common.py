...
# ========================
# 快速版本的编码函数（预编译struct，避免重复创建）
# ========================
_float_pack = struct.Struct('>f').pack
_uint32_unpack = struct.Struct('>I').unpack

def float_to_hex_fast(f):
    """快速版本：直接返回 4 字节 little-endian"""
    return _uint32_unpack(_float_pack(f))[0].to_bytes(4, 'little')

# Kp 查找表：预计算 0-255 范围内的 Kp 值对应的字节
_kp_cache = {}
def _init_kp_cache():
    for kp_int in range(256):
        for kp_frac in range(10):
            kp = kp_int + kp_frac * 0.1
            base = kp_int
            frac = kp_frac
            if frac < 5:
                val = (base * 32) + frac * 3
            ...
            _kp_cache[(kp_int, kp_frac)] = val.to_bytes(2, 'little')
_init_kp_cache()

def kp_to_hex_fast(Kp):
    """快速版本：使用查找表"""
    kp_int = int(Kp)
    kp_frac = int(round((Kp - kp_int) * 10))
    if kp_frac >= 10:
        kp_frac = 9
    key = (kp_int, kp_frac)
    if key in _kp_cache:
...
# Kd 查找表
_kd_frac_map = {0: 0x0, 1: 0x1, 2: 0x3, 3: 0x4, 4: 0x6, 5: 0x8, 6: 0x9, 7: 0xb, 8: 0xc, 9: 0xe}
_kd_cache = {}
def _init_kd_cache():
    for kd_int in range(256):
        for kd_frac in range(10):
            hex_frac = _kd_frac_map.get(kd_frac, 0)
            val = (kd_int << 4) | hex_frac
            _kd_cache[(kd_int, kd_frac)] = val.to_bytes(2, 'little')
_init_kd_cache()

def kd_to_hex_fast(Kd):
    """快速版本：使用查找表"""
    kd_int = int(Kd)
    kd_frac = int(round((Kd - kd_int) * 10))
    if kd_frac >= 10:
        kd_frac = 9
    key = (kd_int, kd_frac)
    if key in _kd_cache:
        return _kd_cache[key]
    # fallback
    return kd_to_hex(Kd)

def tau_to_hex_fast(tau):
    """快速版本：减少函数调用"""
    tau = round(tau, 2)
    ...
    # fraction_to_hex inline
    if fractional_part == 0.0:
        hex_frac = 0
    else:
        hex_frac = int(fractional_part * 256)
        if neg:
            hex_frac = 255 + hex_frac + 1
    
    return bytes([hex_frac & 0xFF, integer_part & 0xFF])
...
def hex_to_float_2B(data, start_index, end_index):
    """
    Convert 2 bytes to float with one decimal place accuracy.
    Only one decimal place accuracy is guaranteed.
    """
    if (end_index - start_index) != 1:
        raise ValueError("Input data must contain exactly two bytes (start_index and end_index should differ by 1).")
    
    # Interpret the two bytes as a little-endian uint16_t
    # Note: data[end_index] is the high byte, data[start_index] is the low byte
    val = (data[end_index] << 8) | data[start_index]
    ...
    if remainder < 15:
        ...
        frac = (remainder - 4) / 3.0 + 1.0
    
    result = float(base) + frac * 0.1
    return result
