...
class UnitreeDogNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info('Unitree机器狗节点已启动')
    ...
    def _publish_sensor_data(self):
        ...
        try:
            ...
            # 四元数 (w, x, y, z)
            imu_msg.orientation.w = self.lstate.imu.quaternion[0]
            imu_msg.orientation.x = self.lstate.imu.quaternion[1]
            imu_msg.orientation.y = self.lstate.imu.quaternion[2]
            imu_msg.orientation.z = self.lstate.imu.quaternion[3]
            
            # 角速度 (rad/s)
            imu_msg.angular_velocity.x = self.lstate.imu.gyroscope[0]
            imu_msg.angular_velocity.y = self.lstate.imu.gyroscope[1]
            imu_msg.angular_velocity.z = self.lstate.imu.gyroscope[2]
            
            # 加速度 (m/s^2)
            imu_msg.linear_acceleration.x = self.lstate.imu.accelerometer[0]
            imu_msg.linear_acceleration.y = self.lstate.imu.accelerometer[1]
            imu_msg.linear_acceleration.z = self.lstate.imu.accelerometer[2]
            ...
            # 提取12个主要关节的状态
            for i in range(12):
                motor_state = self.lstate.motorState[i]
                joint_state_msg.position.append(motor_state.q)
                joint_state_msg.velocity.append(motor_state.dq)
                joint_state_msg.effort.append(motor_state.tauEst)
